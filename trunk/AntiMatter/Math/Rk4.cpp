...
namespace AntiMatter
{
	...
	)
	{
		...
		REAL k1		= f( tn, xn );
		REAL k2		= f( tn + (0.5f*dt), xn + (0.5f*k1) );
		REAL k3		= f( tn + (0.5f*dt), xn + (0.5f*k2) );
		REAL k4		= f( tn + dt, xn + k3 );

		REAL dx		= ( (1.0f/6.0f) * ( k1 + (2.0f*k2) + (2.0f*k3) + k4 )) * dt;		

		return x0 + dx;
	}

	void Rk4::IntegrateSteps(	
		std::vector<REAL> & vResults, 
		const MotionEquation & f, 
		const REAL dt, 
		const REAL t0, 
		const REAL tn, 
